...
> - 适用 SDK 版本：`v1.0.0`
> - **图片版权与来源声明**：封面机体形象来源于官方 SDK 资源；动作调度拓扑架构图与双臂自由度映射图为本文结合系统机制原创绘制。
...
![灵犀 X2 双臂 7 自由度与动作空间拓扑](../pictures/lingxi_x2_motion/arm_joints_diagram.png)

每个手臂内部由 7 个精密关节级联而成（肩部 3 轴 + 肘部 1 轴 + 腕部 3 轴）。预设动作之所以运行顺滑，核心就在于底层插值算法已经在三维空间中预先标定了安全运动包络线，既规避了机械臂与机身胸腔的自碰撞风险，又通过 WBC 全身动力学控制器实时向双腿分配反向平衡力矩，彻底消除上肢挥舞时的失稳隐患。
