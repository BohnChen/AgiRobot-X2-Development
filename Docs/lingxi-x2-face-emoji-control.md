...
很多开发者第一次翻看官方 SDK 时，都会疑惑：**为什么代码包里搜不到任何表情的 PNG 图片或视频文件？**

原因在于灵犀 X2 的多板分工架构：屏幕渲染服务运行在**交互计算单元（PC3，10.0.1.42）**，其底层的动画资源（SVGA/MP4 文件）直接固化在机器人内部的系统分区中，对外仅向开发板暴露了轻量的整数编号（`emotion_id`）。因此在本地离线环境下确实“查不到”原画文件；但在真机运行时，屏幕会呈现非常细腻的高光 OLED 点阵动效。

灵犀 X2 的头部屏幕以“**高亮银白科技几何双眼**”为主视觉符号。整个视觉系统通过眼睛弧度、形状缩放以及动态粒子，传达出机器人的当前意图与情绪状态：

![灵犀 X2 头部屏经典表情效果示意](../pictures/lingxi_x2_emoji/face_emojis_preview.png)

1. **平静·注视（ID: 10 / 50）**：默认待机状态。双眼呈现饱满的高亮银白胶囊形态，伴随低频的自然眨眼动作。
2. **快乐·笑眼（ID: 90）**：交互积极反馈。双眼弯成两道向上弯曲的高亮银白月牙笑弧，常用于打招呼或语音识别成功响应。
3. **加倍开心·星星眼（ID: 100）**：狂喜或深度赞许。眼球中心绽放出高亮璀璨的四角星芒，表达热烈情绪。
4. **思考·流转（ID: 170）**：大模型推理中。双眼切换为环形点阵流光粒子，清晰提示用户“系统正在处理、请稍候”。
5. **疑惑·高低眉（ID: 130）**：语音识别置信度不足或指令不明确时触发。一侧眼睛微眯平直，另一侧睁大挑起，呈现生动的困惑神情。
6. **异常·告警（ID: 70）**：电机过温、触觉碰撞或系统报障。屏幕色调转为鲜亮高光的双叉红色“X”形态。
